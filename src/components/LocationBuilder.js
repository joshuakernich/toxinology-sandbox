...
const LocationDD = ({value}) => (
  <select value={value} class={style.locationdd}>
...
const LocationBuilder = (props) => {
  ...
  const [list, setList] = useState(['AU']);

  const doNewLocation = ()=> {
      setList(list.concat('Current Location'));
  }

  return <div class={style.locationbuilder}>
    {list.map((loc)=> <LocationDD value={loc}/>)}
